...
def register():
    ...
    if auth.get_logged_in_user(session) is not None:
        ...
        if auth.does_user_exist(username):
            ...
            return render_template("register_success.html", seed=(auth.get_user_data(username)["seed"]).hex())
...
@app.route("/e-chap", methods=['GET'])
def echap():
    result = eChap.process(request)
    
    if result[0] is ECHAPRESULT.OK:
        return result[1]
    else:
...
def elogin():
    ...
    result = eChap.try_login(request, session)
...
if __name__ == '__main__':
    ...
    app.run(debug=False, host='localhost', port=443, ssl_context=("certs/cert.pem", "certs/key.pem"))
