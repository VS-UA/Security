...
from app_cryptography import random_challenge
...
class Auth:
    ...
    def register(self, usr: str, password: str, mail: str):
        ...
        if self.is_username_valid(usr) and self.is_password_valid(password) and self.is_email_valid(mail):
            ...
            if self.db.get_user(usr) is not None:
                return False
            self.db.create_user(usr, password_hash.decode(), password, seed, mail)
            return True
        else:
            return False

    def __new_seed(self):
        return random_challenge(16)
    ...
    def login_chap(self, username: str, session: SessionMixin):
    ...
    def get_user_data(self, username: str):
        if self.is_username_valid(username) and self.does_user_exist(username):
